...
namespace BlogEngine.MigrationService;
    ...
    : BackgroundService
{
    ...
    private static async Task SeedDataAsync(ApplicationDbContext dbContext, IServiceScope scope, CancellationToken cancellationToken)
    {
        if (!dbContext.Roles.Any())
        {
            var roleManager = scope.ServiceProvider.GetRequiredService<RoleManager<Role>>();
            await roleManager.CreateAsync(new Role { Name = SharedConstants.User });
            await roleManager.CreateAsync(new Role { Name = SharedConstants.Administrator });
        }
    }
}